...
class QValueRenderWrapper(Wrapper):
    ...
    def __init__(self, env, model, render_fps=4, window_size=600):
        ...
        try:
            with resources.path('avocado_run.wrappers.sprites', 'arrow.png') as arrow_path:
                self.arrow_image = pygame.image.load(
                    str(arrow_path)).convert_alpha()
            with resources.path('avocado_run.wrappers.sprites', 'rectangle.png') as rectangle_path:
                self.rectangle_image = pygame.image.load(
                    str(rectangle_path)).convert_alpha()
